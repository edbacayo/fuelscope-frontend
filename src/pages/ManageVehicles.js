...
const ManageVehicles = () => {
    ...
    const handleFieldChange = (id, field, value) => {
        setVehicles((prev) =>
            prev.map((v) => {
                ...
                const updated = {
                    ...v,
                    ...(field === "name" ? { editedName: value } : { editedOdometer: value }),
                };
                updated.hasChanged =
                    updated.editedName !== v.name || Number(updated.editedOdometer) !== v.odometer;
                return updated;
            })
        );
    };
    ...
    const handleDelete = async (id, name) => {
        if (
            window.confirm(
                `Deleting '${name}' will remove all related data. This cannot be undone. Continue?`
            )
        ) {
            try {
        ...
        <div className="container mt-4 position-relative">
            ...
            <div className="toast-container position-fixed bottom-0 end-0 p-3">
                {toasts.map((t) => (
                    <div
                        key={t.id}
                        className={`toast align-items-center text-white bg-${t.variant} border-0 show mb-2`}
                        role="alert"
                    >
                            ...
                            <tr key={v._id}>
                                <td>
                                    <input
                                        ...
                                        value={v.editedName}
                                        onChange={(e) =>
                                            handleFieldChange(v._id, "name", e.target.value)
                                        }
                                    />
                                </td>
                                <td>
                                    <input
                                        ...
                                        onChange={(e) =>
                                            handleFieldChange(v._id, "odometer", e.target.value)
                                        }
                                    />
                                </td>
                                <td>
